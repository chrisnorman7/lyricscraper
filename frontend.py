...
class LyricsFrame(wx.Frame):
 ...
 def __init__(self):
  ...
  s1.Add(wx.StaticText(p, label = '&Artist'), 0, wx.GROW)
  self.artist = wx.TextCtrl(p)
  s1.Add(self.artist, 1, wx.GROW)
  s.Add(s1, 0, wx.GROW)
  s2 = wx.BoxSizer(wx.HORIZONTAL)
  s2.Add(wx.StaticText(p, label = '&Title'), 0, wx.GROW)
  self.title = wx.TextCtrl(p)
  s2.Add(self.title, 1, wx.GROW)
  s.Add(s2, 0, wx.GROW)
 ...
 def get_lyrics(self, event):
  ...
  if not artist:
   ...
   if lyrics is None:
    ...
    self.lyrics.SetValue('%s - %s (%s)\n\n%s' % (artist, title, lyrics.engine, lyrics.lyrics))
